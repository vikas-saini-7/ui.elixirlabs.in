...
export interface OnThisPageProps extends Omit<
...
> {
  ...
  /** Visual style of list */
  variant?: OnThisPageVariant;

  /** Custom icon renderer (overrides bullet) */
  icon?: (
    item: OnThisPageItem,
    index: number,
    isActive: boolean,
...
const OnThisPage = React.forwardRef<HTMLElement, OnThisPageProps>(
  (
    {
      className,
      items,
    ...
    const getLevelStyles = (level?: number) => {
      ...
      if (l === 2) return "ml-4 text-muted-foreground";
      if (l === 3) return "ml-8 text-muted-foreground";
      return "";
    };
    ...
    const renderDefaultIcon = () => {
      if (variant === "bullet") {
        return (
          <span className="mt-[6px] h-1.5 w-1.5 rounded-full bg-muted-foreground" />
        );
      }
      return null;
    };
    ...
    return (
      <nav ref={ref} className={cn("w-full text-sm", className)} {...props}>
        {label && (
        ...
        <ul className="space-y-2">
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                {icon ? icon(item, index, isActive) : renderDefaultIcon()}
